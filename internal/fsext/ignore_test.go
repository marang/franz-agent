...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .franzignore file that ignores .log files
	require.NoError(t, os.WriteFile(".franzignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .franzignore file
	crushignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".franzignore"), []byte(crushignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .franzignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .franzignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .franzignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".franzignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .franzignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .franzignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .franzignore")
