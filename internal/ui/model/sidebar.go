...
func (m *UI) modelInfo(width int) string {
	...
	if m.planModeEnabled {
		if reasoningInfo == "" {
			reasoningInfo = "Planning Mode On"
		} else {
			reasoningInfo += " · Planning Mode On"
		}
	}
...
func (m *UI) drawSidebar(scr uv.Screen, area uv.Rectangle) {
	...
	sidebarParts := layout.Vertical(layout.Len(lipgloss.Height(sidebarHeader)), layout.Fill(1)).Split(m.layout.sidebar)
	_, remainingHeightArea := sidebarParts[0], sidebarParts[1]
