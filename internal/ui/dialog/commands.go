...
func (c *Commands) defaultCommands() []*CommandItem {
	...
	if c.hasSession {
		items = append(items, NewCommandItem(c.com.Styles, "summarize", "Summarize Session", "", ActionSummarize{SessionID: c.sessionID}))
	}
	...
	if agentCfg, ok := cfg.Agents[config.AgentCoder]; ok {
		...
		if providerCfg != nil && model != nil && model.CanReason {
			...
			if model.CanReason && len(model.ReasoningLevels) == 0 {
				...
				items = append(items, NewCommandItem(c.com.Styles, "toggle_thinking", status+" Thinking Mode", "", ActionToggleThinking{}))
			}

			// OpenAI models: reasoning effort dialog
			if len(model.ReasoningLevels) > 0 {
				items = append(items, NewCommandItem(c.com.Styles, "select_reasoning_effort", "Select Reasoning Effort", "", ActionOpenDialog{
	...
	if c.windowWidth >= sidebarCompactModeBreakpoint && c.hasSession {
		items = append(items, NewCommandItem(c.com.Styles, "toggle_sidebar", "Toggle Sidebar", "", ActionToggleCompactMode{}))
	}
	if c.hasSession {
		...
		if model != nil && model.SupportsImages {
			items = append(items, NewCommandItem(c.com.Styles, "file_picker", "Open File Picker", "ctrl+f", ActionOpenDialog{
	...
	if hasConfiguredExternalEditor(c.com.Config()) {
		items = append(items, NewCommandItem(c.com.Styles, "open_external_editor", "Open External Editor", "ctrl+o", ActionExternalEditor{}))
	}
	...
	if !cfg.IsDockerMCPEnabled() && c.dockerMCPAvailable != nil && *c.dockerMCPAvailable {
		items = append(items, NewCommandItem(c.com.Styles, "enable_docker_mcp", "Enable Docker MCP Catalog", "", ActionEnableDockerMCP{}))
	}

	// Add disable Docker MCP command if it's currently enabled
	if cfg.IsDockerMCPEnabled() {
		items = append(items, NewCommandItem(c.com.Styles, "disable_docker_mcp", "Disable Docker MCP Catalog", "", ActionDisableDockerMCP{}))
	}
	...
	if c.hasTodos || c.hasQueue {
		...
		items = append(items, NewCommandItem(c.com.Styles, "toggle_pills", label, "ctrl+t", ActionTogglePills{}))
	}
	...
	items = append(items, NewCommandItem(c.com.Styles, "toggle_notifications", notificationLabel, "", ActionToggleNotifications{}))

	items = append(items,
		NewCommandItem(c.com.Styles, "toggle_yolo", "Toggle Yolo Mode", "ctrl+y", ActionToggleYoloMode{}),
		NewCommandItem(c.com.Styles, "toggle_plan_mode", "Toggle Planning Mode", "ctrl+shift+p", ActionTogglePlanMode{}),
		NewCommandItem(c.com.Styles, "skills_manager", "Skills Manager", "", ActionOpenDialog{
			DialogID: SkillsSHSearchID,
		}),
	...
	items = append(items, NewCommandItem(c.com.Styles, "toggle_transparent", transparentLabel, "", ActionToggleTransparentBackground{}))
...
func hasConfiguredExternalEditor(cfg *config.Config) bool {
	if cfg != nil && cfg.Options != nil && strings.TrimSpace(cfg.Options.Editor) != "" {
		return true
	}

	// NOTE: Keep this aligned with github.com/charmbracelet/x/editor lookup:
	// app-specific env var (FRANZ_EDITOR), then VISUAL/EDITOR fallbacks.
	return strings.TrimSpace(os.Getenv("FRANZ_EDITOR")) != "" ||
