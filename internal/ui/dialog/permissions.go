...
const (
	...
	PermissionDiscuss         PermissionAction = "discuss"
...
type Permissions struct {
	...
	selectedOption int // 0: Allow, 1: Allow for session, 2: Discuss, 3: Deny
...
func defaultPermissionsKeyMap() permissionsKeyMap {
	return permissionsKeyMap{
		...
		Discuss: key.NewBinding(
			key.WithKeys("c", "C", "ctrl+c"),
			key.WithHelp("c", "discuss"),
		),
		Close: CloseKey,
...
func (p *Permissions) HandleMsg(msg tea.Msg) Action {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, p.keyMap.Discuss):
			return p.respond(PermissionDiscuss)
...
func (p *Permissions) renderButtons(contentWidth int) string {
	buttons := []common.ButtonOpts{
		...
		{Text: "Discuss", UnderlineIndex: 0, Selected: p.selectedOption == 2},
