// Package logo renders the Franz wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the Franz title art.
type Opts struct {
	FieldColor   color.Color // diagonal lines
	TitleColorA  color.Color // left gradient ramp point
	TitleColorB  color.Color // right gradient ramp point
	CharmColor   color.Color // Attribution text color
	VersionColor color.Color // Version text color
	Width        int         // width of the rendered logo, used for truncation
}

// Render renders the Franz logo. Set the argument to true to render the narrow
// version, intended for use in a sidebar.
//
// The compact argument determines whether it renders compact for the sidebar
// or wider for the main pane.
func Render(s *styles.Styles, version string, compact bool, o Opts) string {
	const attribution = "Based on Crush"

	fg := func(c color.Color, s string) string {
		return lipgloss.NewStyle().Foreground(c).Render(s)
	}

	// Title and branding hierarchy.
	const wordmarkASCII = `┏━╸┏━┓┏━┓┏┓╻╺━┓
┣╸ ┣┳┛┣━┫┃┗┫┏━┛
╹  ╹┗╸╹ ╹╹ ╹┗━╸`
	wordmark := lipgloss.NewStyle().Foreground(o.TitleColorA).Bold(true).Render(wordmarkASCII)
	wordmarkWidth := lipgloss.Width(wordmarkASCII)
	rightAlign := func(v string, width int) string {
		pad := max(0, width-lipgloss.Width(v))
		return strings.Repeat(" ", pad) + v
	}

	basedOn := rightAlign(fg(o.CharmColor, attribution), wordmarkWidth)
	maxVersionWidth := max(0, wordmarkWidth-1)
	version = ansi.Truncate(version, maxVersionWidth, "…") // truncate version if too long.
	metaRow := ""
	if strings.TrimSpace(version) != "" {
		metaRow = rightAlign(fg(o.VersionColor, version), wordmarkWidth)
	}

	parts := []string{wordmark, basedOn}
	if metaRow != "" {
		parts = append(parts, metaRow)
	}
	branding := strings.TrimSpace(strings.Join(parts, "\n"))
	brandingWidth := wordmarkWidth

	// Narrow version.
	if compact {
		targetWidth := brandingWidth
		if o.Width > 0 {
			targetWidth = max(targetWidth, o.Width)
		}

		field := styles.ApplyForegroundGrad(s, strings.Repeat(diag, targetWidth), o.TitleColorA, o.TitleColorB)
		lines := strings.Split(branding, "\n")
		remain := max(0, targetWidth-brandingWidth)
		rightStripe := ""
		if remain > 1 {
			rightStripe = " " + styles.ApplyForegroundGrad(s, strings.Repeat(diag, remain-1), o.TitleColorA, o.TitleColorB)
		} else if remain == 1 {
			rightStripe = " "
		}
		for i, line := range lines {
			lines[i] = ansi.Truncate(line+rightStripe, targetWidth, "")
		}
		body := strings.Join(lines, "\n")
		return strings.Join([]string{field, field, body, field, ""}, "\n")
	}

	fieldHeight := lipgloss.Height(branding)
	...
	// Right field.
	rightWidth := max(15, o.Width-brandingWidth-leftWidth-2) // 2 for the gap.
	rightField := new(strings.Builder)
	for range fieldHeight {
		fmt.Fprint(rightField, fg(o.FieldColor, strings.Repeat(diag, rightWidth)), "\n")
	}

	// Return the wide version.
	const hGap = " "
	logo := lipgloss.JoinHorizontal(lipgloss.Top, leftField.String(), hGap, branding, hGap, rightField.String())
	if o.Width > 0 {
...
// SmallRender renders a smaller version of the Franz logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(t *styles.Styles, width int) string {
	title := t.Base.Foreground(t.LogoCharmColor).Render("Based on Crush")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad(t, "Franz", t.LogoTitleColorA, t.LogoTitleColorB))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "Franz"
	if remainingWidth > 0 {
		lines := strings.Repeat("╱", remainingWidth)
		title = fmt.Sprintf("%s %s", title, t.Base.Foreground(t.LogoFieldColor).Render(lines))
	}
	return title
}
