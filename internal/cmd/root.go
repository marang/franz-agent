...
import (
	...
	"github.com/marang/franz-agent/internal/db"
	"github.com/marang/franz-agent/internal/event"
	"github.com/marang/franz-agent/internal/projects"
	"github.com/marang/franz-agent/internal/ui/common"
	ui "github.com/marang/franz-agent/internal/ui/model"
...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom franz-agent data directory")
...
var rootCmd = &cobra.Command{
	Use:   "franz-agent",
	Short: "A terminal-first AI assistant for software development (unofficial adapted client)",
	Long:  "A glamorous, terminal-first AI assistant for software development and adjacent tasks (unofficial adapted client)",
	Example: `
# Run in interactive mode
franz-agent

# Run non-interactively
franz-agent run "Guess my 5 favorite Pokémon"

# Run a non-interactively with pipes and redirection
cat README.md | franz-agent run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
franz-agent --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
franz-agent --yolo

# Run with custom data directory
franz-agent --data-dir /path/to/custom/.franz-agent

# Continue a previous session
franz-agent --session {session-id}

# Continue the most recent session
franz-agent --continue
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("Franz crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/marang/franz-agent/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("FRANZ_DISABLE_METRICS")); v {
		return false
	}
