...
var dirsCmd = &cobra.Command{
	Use:   "dirs",
	Short: "Print directories used by Franz",
	Long: `Print the directories where Franz stores its configuration and data files.
...
franz-agent dirs config

# Print only the data directory
franz-agent dirs data
  `,
...
var configDirCmd = &cobra.Command{
	Use:   "config",
	Short: "Print the configuration directory used by Franz",
	Run: func(cmd *cobra.Command, args []string) {
		cmd.Println(filepath.Dir(config.GlobalConfig()))
	},
}
...
var dataDirCmd = &cobra.Command{
	Use:   "data",
	Short: "Print the datauration directory used by Franz",
	Run: func(cmd *cobra.Command, args []string) {
		cmd.Println(filepath.Dir(config.GlobalConfigData()))
	},
}
