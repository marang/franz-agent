...
func TestConfig_setDefaults(t *testing.T) {
	...
	require.Equal(t, filepath.Join("/tmp", ".franz-agent"), cfg.Options.DataDirectory)
...
func TestLookupConfigsDefaultOrder(t *testing.T) {
	t.Parallel()

	cfgPaths := lookupConfigs("/path/that/does/not/exist")
	require.GreaterOrEqual(t, len(cfgPaths), 3)
	require.Equal(t, GlobalConfig(), cfgPaths[0])
	require.Equal(t, GlobalHomeConfig(), cfgPaths[1])
	require.Equal(t, GlobalConfigData(), cfgPaths[2])
}
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("FRANZ_DISABLE_DEFAULT_PROVIDERS", "true")
...
func TestConfig_setDefaultsDisableProviderAutoUpdateEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		...
		require.True(t, cfg.Options.DisableProviderAutoUpdate)
	})
	...
	t.Run("does not use legacy environment variable", func(t *testing.T) {
		...
		require.False(t, cfg.Options.DisableProviderAutoUpdate)
	})
}
