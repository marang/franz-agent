...
import (
	...
	"github.com/marang/franz-agent/internal/env"
...
func PushPopCrushEnv() func() {
	...
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "FRANZ_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "FRANZ_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("FRANZ_"+ev))
	}
	...
	for _, p := range knownProviders {
		...
		case catwalk.InferenceProvider(openai_codex.ProviderID):
			...
			if _, ok := prepared.ProviderOptions["instructions"]; !ok {
				prepared.ProviderOptions["instructions"] = "You are Franz, a terminal-first coding assistant."
			}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("IZO_AGENT_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("FRANZ_DISABLE_DEFAULT_PROVIDERS"); ok {
...
func lookupConfigs(cwd string) []string {
	// Prepend default global config paths.
	//
	// Order:
	// 1) ~/.config/franz-agent/franz-agent.json
	// 2) ~/.franz-agent/franz-agent.json
	// 3) ~/.local/share/franz-agent/franz-agent.json
	configPaths := []string{
		GlobalConfig(),
		GlobalHomeConfig(),
		GlobalConfigData(),
...
func loadFromBytes(configs [][]byte) (*Config, error) {
	...
	data, err := jsonmerge.MergeDocuments(configs)
	if err != nil {
		return nil, err
	}
	var config Config
...
func GlobalConfig() string {
	if franzGlobal := os.Getenv("FRANZ_GLOBAL_CONFIG"); franzGlobal != "" {
		return filepath.Join(franzGlobal, fmt.Sprintf("%s.json", appName))
	}
	return filepath.Join(home.Config(), appName, fmt.Sprintf("%s.json", appName))
}

// GlobalHomeConfig returns the fallback home-directory configuration file path
// for the application, e.g. ~/.franz-agent/franz-agent.json.
func GlobalHomeConfig() string {
...
func GlobalConfigData() string {
	if franzData := os.Getenv("FRANZ_GLOBAL_DATA"); franzData != "" {
		return filepath.Join(franzData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/franz-agent/`
	// for linux and macOS, it should be in `$HOME/.local/share/franz-agent/`
...
func GlobalSkillsDirs() []string {
	if franzSkills := os.Getenv("FRANZ_SKILLS_DIR"); franzSkills != "" {
		return []string{franzSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/franz-agent`.
...
// ProjectSkillsDir returns the default project directories for which Franz
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, "."+appName, "skills"),
