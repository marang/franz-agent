...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads franz-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "franz://skills/franz-config/SKILL.md",
		})
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "Franz Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "franz://skills/nonexistent/SKILL.md",
		})
		require.NoError(t, err)
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "franz-config", meta.ResourceName)
