...
import (
	...
	"github.com/marang/franz-agent/internal/agent/prompt"
	"github.com/marang/franz-agent/internal/agent/tools"
	"github.com/marang/franz-agent/internal/config"
	"github.com/marang/franz-agent/internal/filetracker"
	"github.com/marang/franz-agent/internal/history"
	"github.com/marang/franz-agent/internal/jsonmerge"
	"github.com/marang/franz-agent/internal/log"
...
type Coordinator interface {
	...
	RunWithOptions(ctx context.Context, sessionID, prompt string, opts RunOptions, attachments ...message.Attachment) (*fantasy.AgentResult, error)
...
// RunWithOptions implements Coordinator.
func (c *coordinator) RunWithOptions(ctx context.Context, sessionID string, prompt string, opts RunOptions, attachments ...message.Attachment) (*fantasy.AgentResult, error) {
	...
	mergedOptions = applySessionProviderOptimizations(mergedOptions, model, providerCfg, sessionID)
...
func getProviderOptions(model Model, providerCfg config.ProviderConfig, sessionID string) fantasy.ProviderOptions {
	...
	got, err := jsonmerge.MergeDocuments([][]byte{
		catwalkOpts,
		providerCfgOpts,
		cfgOpts,
	})
	...
	case openai.Name, azure.Name:
		...
		if openai.IsResponsesModel(model.CatwalkCfg.ID) {
			...
			if shouldEnablePromptCacheKey(providerCfg.ID, sessionID) {
...
func applySessionProviderOptimizations(
	...
	perSession := getProviderOptions(model, providerCfg, sessionID)
	merged := make(fantasy.ProviderOptions, len(base)+len(perSession))
	maps.Copy(merged, base)
	maps.Copy(merged, perSession)
	return merged
}

func shouldEnablePromptCacheKey(providerID, sessionID string) bool {
	if sessionID == "" {
		return false
	}
	if !envBoolDefaultTrue("FRANZ_FEATURE_PROMPT_CACHE_KEY") {
		return false
	}
	if isSubscriptionProvider(providerID) {
		return true
	}
	raw := strings.TrimSpace(os.Getenv("FRANZ_PROMPT_CACHE_PROVIDER_IDS"))
	if raw == "" {
		return false
	}
	for _, token := range strings.Split(raw, ",") {
		if strings.TrimSpace(token) == providerID {
			return true
		}
	}
	return false
...
func isSubscriptionProvider(providerID string) bool {
	...
	raw := strings.TrimSpace(os.Getenv("FRANZ_SUBSCRIPTION_PROVIDER_IDS"))
	if raw == "" {
		return false
	}
	for _, token := range strings.Split(raw, ",") {
...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent) ([]fantasy.AgentTool, error) {
	...
	if isSubscriptionProvider(providerID) {
		if envBoolDefaultFalse("FRANZ_FEATURE_SUBSCRIPTION_ESSENTIAL_TOOLS") {
			filteredTools = tools.KeepEssentialTools(filteredTools)
		}
		if envBoolDefaultTrue("FRANZ_FEATURE_COMPACT_TOOL_PAYLOAD") {
			filteredTools = tools.CompactToolsForModelPayload(filteredTools)
		}
	}
...
func (c *coordinator) buildOpenaiProvider(baseURL, apiKey string, headers map[string]string) (fantasy.Provider, error) {
	...
	httpClient := c.openAIHTTPClient(headers)
	if httpClient != nil {
...
func (c *coordinator) openAIHTTPClient(headers map[string]string) *http.Client {
	enableSessionHeader := envBoolDefaultTrue("FRANZ_FEATURE_SESSION_ID_HEADER")
	debugHTTP := c.cfg.Config().Options.Debug
	if !enableSessionHeader && !debugHTTP {
		return nil
	}
	...
	if enableSessionHeader {
		client.Transport = wrapSessionHeaderTransport(client.Transport)
	}
	return client
}
