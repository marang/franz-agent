// Package agent is the core orchestration layer for Franz AI agents.
//
...
import (
	...
	"github.com/marang/franz-agent/internal/stringext"
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (*fantasy.AgentResult, error) {
	...
	if call.PlanMode {
		agentTools = nil
		systemPrompt += "\n\n<planning_mode>\nPlanning mode is enabled.\nReturn only an actionable plan and clarifying questions when needed.\nDo not call tools, do not edit files, and do not execute commands.\n</planning_mode>"
	}
	...
	logPromptBudgetDebug(call.SessionID, largeModel, systemPrompt, promptPrefix, call.ProviderOptions, history, agentTools)
	...
	if err != nil {
		...
		for _, tc := range toolCalls {
			...
			} else if isPermissionDiscussErr {
				content = "Permission moved to discussion with the user"
			...
			toolResult := message.ToolResult{
				...
				Name:       tc.Name,
				Content:    content,
				IsError:    !isPermissionDiscussErr,
			}
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonCanceled, "Permission moved to discussion", "")
		} else if isPermissionErr {
			...
			if subscription.HasFetcher(largeModel.ModelCfg.Provider) && providerErr.StatusCode == 401 {
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("FRANZ_DISABLE_ANTHROPIC_CACHE")); t {
...
If not, please feel free to ignore. Again do not mention this message to the user.`,
	...
	for _, m := range msgs {
		...
		if m.Role == message.Tool {
			msg, ok := filterOrphanedToolResults(m, knownToolCallIDs)
			if !ok {
				continue
			}
...
func summarizeThreshold(contextWindow int64, isSubscription bool) int64 {
	if isSubscription && envBoolDefaultFalseAgent("FRANZ_FEATURE_SUBSCRIPTION_EARLY_SUMMARIZE") {
		threshold := int64(float64(contextWindow) * 0.30)
		if threshold < 40_000 {
			threshold = 40_000
		}
		return threshold
...
func envBoolDefaultFalseAgent(key string) bool {
...
func envBoolDefaultTrueAgent(key string) bool {
	value := strings.TrimSpace(os.Getenv(key))
	if value == "" {
		return true
	}
...
func logPromptBudgetDebug(
	...
	if !envBoolDefaultFalseAgent("FRANZ_DEBUG_PROMPT_BUDGET") {
		return
	}

	historyChars := 0
	for _, msg := range history {
		historyChars += estimateMessageChars(msg)
	}
	...
	for _, tool := range agentTools {
		info := tool.Info()
		toolDescChars += len(info.Description)
		if encoded, err := json.Marshal(info.Parameters); err == nil {
			toolSchemaChars += len(encoded)
		}
	}
	...
	slog.Info(
		...
		"system_prefix_chars", len(promptPrefix),
		"tools_count", len(agentTools),
		"tool_description_chars", toolDescChars,
		"tool_schema_chars", toolSchemaChars,
		"provider_options_keys", len(providerOptions),
	)
}

func estimateMessageChars(msg fantasy.Message) int {
	size := len(msg.Role)
	for _, part := range msg.Content {
		switch p := part.(type) {
		...
		case fantasy.ReasoningPart:
			size += len(p.Text)
			if b, err := json.Marshal(p.ProviderOptions); err == nil {
				size += len(b)
			}
		case fantasy.ToolCallPart:
			size += len(p.ToolCallID) + len(p.ToolName) + len(p.Input)
		case fantasy.ToolResultPart:
			size += len(p.ToolCallID)
			if b, err := json.Marshal(p.Output); err == nil {
				size += len(b)
			}
		case fantasy.FilePart:
			size += len(p.Filename) + len(p.MediaType) + len(p.Data)
		default:
