# Franz Development Guide

## Project Overview

Franz ist hier ein inoffizieller, adaptierter Client.

Franz is a terminal-based AI coding assistant built in Go by
...
The module path is `github.com/marang/franz-agent`.
...
internal/
  ...
  config/
    ...
    load.go                        franz-agent.json loading and validation
...
- **Context files**: Franz reads AGENTS.md, CRUSH.md, CLAUDE.md, GEMINI.md
...
- **Build (dist binary)**: `make build` (alias: `make build-dist`)
- **Run**: `make run`
- **Test**: `make test` or `go test ./...` (run single test:
  `go test ./internal/llm/prompt -run TestGetContextFromPaths`)
- **Update Golden Files**: `go test ./internal/ui/diffview -update` (or
  `make test-update-golden`) to regenerate `.golden` files for packages that
  support the `-update` flag.
- **Format**: `gofumpt -w .`
...
- **Constants**: Use typed constants with iota for enums, group in const
  blocks.
- **No magic strings**: Do not hardcode repeated or behavior-defining string
  literals inline. Extract them into named constants (or typed enums/maps)
  close to the owning package/domain.
...
- **Log messages**: Log messages must start with a capital letter (e.g.,
  ...
  - This is enforced by lint tooling in CI/local checks.
...
This repository uses a **fresh-history `main`** branch with no persistent
upstream-lineage branch.

When updating from the original `charmbracelet/crush` repository, use this
workflow:

1. Ensure upstream remote exists:
   - `git remote add upstream git@github.com:charmbracelet/crush.git` (once).
2. Fetch latest upstream:
   - `git fetch upstream`
3. Create a temporary integration branch from upstream:
   - `git checkout -b tmp/upstream-<date> upstream/main`
4. Select changes to bring into Franz:
   - Prefer targeted `git format-patch` + `git am` or `git cherry-pick -x`
     into `main`.
   - For larger updates, split into smaller topical commits before porting.
5. Apply into product branch:
   - `git checkout main`
   - `git cherry-pick -x <commit>` (or apply patch series).
6. Resolve conflicts and run tests.
7. Push `main` and delete temporary integration branch when done.

Important:

- Do **not** merge `upstream/main` directly into `main` (histories are
  intentionally different).
- Keep upstream updates small and reviewable; prefer incremental imports.
